...
export default function SellerProductsPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onMutate: async (deletedId) => {
      // Cancelar queries en curso que puedan afectar
    ...
    onError: (_err, _id, context) => {
      // Si hay error, revertir a datos anteriores
      ...
      <header className="sticky top-0 z-10 bg-white shadow-sm border-b">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-semibold text-gray-900">Panel de Productos</h1>
              <p className="mt-1 text-sm text-gray-600">Gestiona tu catálogo de productos</p>
            </div>
            <button
          ...
          <div className="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 overflow-hidden">
            ...
            {products.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="mt-1 text-sm text-gray-500">Comienza creando un nuevo producto para tu catálogo.</p>
              </div>
            )}
          </div>
        )}
