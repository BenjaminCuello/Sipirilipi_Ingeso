...
const extraArgs = [];
const host = process.env.VITE_DEV_HOST || process.env.HOST;
const port = process.env.VITE_DEV_PORT || process.env.PORT;
if (host) {
...
run(npmCmd, ["run", "dev:serve", ...(extraArgs.length > 0 ? ["--", ...extraArgs] : [])]);
