...
const StatusBadge: React.FC<{ status: OrderStatus }> = ({ status }) => {
    ...
    return <span className={`px-2 py-1 text-xs font-medium rounded-full ${statusStyles[status]}`}>{statusText[status]}</span>
}

// Página principal del historial de órdenes
const OrdersPage: React.FC = () => {
  ...
  if (isLoading) {
    return <div className="p-6 text-center">Cargando historial de órdenes...</div>;
  }

  if (isError) {
    return <div className="p-6 text-center text-red-600">Ocurrió un error al cargar tus órdenes.</div>;
  }
            ...
            <tbody className="divide-y divide-gray-200">
              {orders.map(order => (
                <tr key={order.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-gray-600">{format(new Date(order.createdAt), 'dd MMM yyyy', { locale: es })}</td>
                  {/* CORRECCIÓN: Usando tu función */}
                  <td className="px-6 py-4 whitespace-nowrap text-gray-600">{formatCLPFromCents(order.total)}</td>
                  <td className="px-6 py-4 whitespace-nowrap"><StatusBadge status={order.status} /></td>
                  <td className="px-6 py-4 whitespace-nowrap text-right">
                    <button onClick={() => setSelectedOrder(order)} className="font-medium text-indigo-600 hover:text-indigo-900">
                      Ver Detalle
                    </button>
                  </td>
                </tr>
